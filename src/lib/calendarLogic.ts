...
// Robust mapping helper for Tithi names to Gujarati
...
export const calculateTithi = (inputDate: Date): TithiData => {
  ...
  // Force calculation for Noon to avoid midnight edge cases
  ...
  try {
    ...
    // Pass Sunrise time to Panchang calculation
  ...
  // Derived flags for UI highlighting
  const isAgiyaras = tithiName === "Agiyaras";
  const isPoonam = tithiName === "Poonam";
...
export const getUpcomingAgiyaras = (): { date: Date; tithi: TithiData }[] => {
  ...
  for (const event of foundEvents) {
    ...
    if (uniqueEvents.length >= 4) break;
  }
  return uniqueEvents;
};
