...
export const ErrorState: React.FC<ErrorStateProps> = ({ message, onRetry }) => {
  ...
  return (
    <div className="flex flex-col items-center justify-center py-12 text-center mt-10 px-6">
      ...
      {onRetry && (
        <button onClick={onRetry} className={`flex items-center gap-2 px-5 py-2.5 rounded-xl font-bold transition-transform active:scale-95 ${isDarkMode ? 'bg-white/10 text-white hover:bg-white/20' : 'bg-black/5 text-slate-800 hover:bg-black/10'}`}>
